...
function Suggestions({ message, onSendMessage }) {
  ...
  // Detect response type and suggest relevant follow-ups
  let s;
  const hasCode = lower.includes('```') || lower.includes('function ') || lower.includes('const ') || lower.includes('import ');
  const hasPerson = lower.includes('born') || lower.includes('known for') || lower.includes('career') || lower.includes('famous');

  if (hasCode) {
    // Coding response
    if (lower.includes('react') || lower.includes('component')) s = ['Show advanced example', 'Add TypeScript types'];
    else s = ['Optimize this code', 'Add error handling'];
  } else if (hasPerson) {
    // Person / biography response
    s = ['Tell me more about them', 'What are their achievements?'];
  } else {
    // General knowledge / conversational response
    s = ['Tell me more', 'Explain in simpler terms'];
  }
